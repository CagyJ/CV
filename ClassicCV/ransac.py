...
## RANSAC
iterations = 100
tolernt_sigma = 1  #容忍的偏差
thresh_size = 0.5  #内点数 迭代停止数
...
for i in range(iterations):
    ...
    for index in range(SIZE+ERROR):
        ...
        if abs(PREDICT_Y-RANDOM_Y[index]) < tolernt_sigma:
            total_inliers += 1
    ...
    plt.title(f"RANSAC in Linear Regression: Iter{i+1}, inner pts: {pretotal}")
    plt.scatter(RANDOM_X,RANDOM_Y)
    Y = best_slope*RANDOM_X + best_intercept
    plt.plot(RANDOM_X,Y,'orange')
    plt.pause(0.2)
    plt.clf()
